...
def get_data_loader(img_fname, lbl_fname, bats):
    ...
    lblt = lblt.float()  # Convertir les labels en float pour être sûr qu'ils sont appropriés pour BCELoss
...
for ep in range(epochs):
    ...
    for i, data in enumerate(train_loader):
        ...
        print("Min y:", y.min().item(), "Max y:", y.max().item())
        y = torch.clamp(y, min=0, max=1)

        optimizer.zero_grad()

        output = Net(x)
        #output = torch.sigmoid(output)  # Ajout de la fonction sigmoid pour garantir que la sortie est entre 0 et 1
    ...
    for data in valid_loader:
        ...
        output = Net(x)
        output = torch.sigmoid(output)  # Utilisation de sigmoid pour les prédictions
        output = output.view(-1, 1)
...
for data in train_loader:
    ...
    output = Net(x)
    #output = torch.sigmoid(output)  # Garantit que la sortie est une probabilité
    lossfunc = torch.nn.BCEWithLogitsLoss(reduction='mean')
    output = output.view(-1, 1)
